...
def internet_set_time():
    ...
    # There's currently no timezone support in MicroPython, and the RTC is set
    # in UTC time.
...
class BluetoothSender():
    ...
    def __init__(self, name=setup.ble_name):
    ...
    def send_heartrate_packet(self, raw_data):
        ...
        self.raw_send(
            struct.pack(
                "QII",
                precise_time,
                self.new_data_Rri and self.Rri,
                self.new_data_error and self.error) +
            raw_data)
            ...
            try:
                ...
                if number[0] == "#":  # special case
                    ...
                    if number[1] == "-":
                        ...
                        # will save but not send the Rri, will be sent during
                        # the transfer of heartbyte data
                        ble_sender.save_Rri(simple_Rri)
                        print(simple_Rri)
                        if not state:  # draw only when different state
                            tft.fill(colors.GREEN)
                            state = True
                        # tft.fill(colors.GREEN)
                        tft.text(number[1:], 10, 30,
